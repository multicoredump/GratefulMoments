...
struct StreakCalculator {
    ...
    func calculateStreak(for moments: [Moment]) -> Int {
        ...
        let daysAgoArray = moments
            .reversed()
            .map(\.timestamp) // map applies a function to every element in an array, creating a new array with the transformed results
            .map {
                calendar.dateComponents([.day], from: $0, to: endOfToday)
            }
            .compactMap { $0.day } //compactMap additionally filters the transformed array to return a nonoptional array
